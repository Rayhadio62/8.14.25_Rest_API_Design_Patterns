from app.blueprints.service_tickets import services_tickets_bp 
from .schemas import services_ticket_schema, services_tickets_schema
...
def create_service_ticket():
    ...
    db.session.commit()
    return user_schema.jsonify(new_service_ticket), 201
...
def read_service_tickets():
    service_tickets = db.session.get(Service_Tickets, user_id)
...
def read_service_ticket(service_ticket_id):
    service_ticket = db.session.get(Service_tickets, user_id)
...
def update_service_ticket(service_ticket_id):
    service_ticket = db.session.get(Service_Tickets, user_id) #Query for our s_t to update
    ...
    try:
        service_ticket_data = service_ticket_schema.load(request.json) #Validating updates
    except ValidationError as e:
